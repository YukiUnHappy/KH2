...
tyrano.plugin.kag ={
    ...
    stat:{
        ...
        is_skip:{},
        is_auto:false, // オート再生状態か否か
        is_playing_se:true, // SE(ボイス)が再生状態か否か
        is_auto_wait_completed: false, // オート再生状態待ちの時間経過したか否か
        auto_transition_wait_timer_id: null, // 自動遷移タイマーの識別ID
        auto_transition_func: null, // 遷移処理の関数オブジェクト
    ...
    init:function(){
        ...
        this.kag = this;
        this.ignore_click = false; // テキストエリアのクリックイベントハンドラを無効にするフラグ（時限式）
...
/////////////ゲーム初期化////////////
    ...
    init_game:function(){
        ...
        //システム変数の初期化
        var tmpsf = null; //$.getStorage(this.kag.config.projectID+"_sf");
        ...
        //自動セーブのデータがあるかどうか
        var auto_save_data = null;//$.getStorage(this.kag.config.projectID+"_tyrano_auto_save");
...
//        }
       ...
       ///messege1 ２個目のメッセージレイヤ。ここは指定回数分作成できるようにする必要があるかも
        ...
        // 音声OFFでシナリオに初期遷移(入ったばかり)した際、AUTOを押した際に次のセリフに遷移しない為、設定する
        this.kag.ftag.setAutoTransitionTimerFromTextLength();
    ...
    loadScenario:function(file_name,call_back){
        ...
        // first.ks をシナリオデータとして window経由で読み込ませる
        if (file_name === "first.ks") {
            var result_obj = that.parser.parseScenario(window.scenarioData);
        ...
        // first.ks ではない スクリプトも良み込ませているみたいなので、その場合は元の処理を使うようにしている
    ...
    /**
     * 自動遷移可能な状態か否か
     * @method canAutoTransition
     * @return {Boolean} 遷移可能かどうかを示す値
     */
    canAutoTransition: function() {
      return this.auto_transition_wait_timer_id !== null && this.stat.is_click_text == false && this.stat.is_auto == true;
    },
     ...
     */
    stopTimer: function() {
      if (this.auto_transition_wait_timer_id !== null) {
        clearTimeout(this.auto_transition_wait_timer_id);
        this.auto_transition_wait_timer_id = null;
      }
    },
     ...
     * オート再生のタイマーを開始
     * @method startTimer
     * @param {Integer} delay transitionを実行を遅延する時間(ミリ秒)
     * @param {Function} transition delayの時間経過した時に呼び出す関数オブジェクト
     */
    startTimer: function(delay, transition) {
      var self = this;
      self.stat.auto_transition_func = transition;
      self.auto_transition_wait_timer_id = setTimeout(function() {
        self.stat.is_auto_wait_completed = true;
        if (self.canAutoTransition() == true) {
          self.startIgnoreClickTimer(600);
          self.execTransition();
        }
      }, delay);
    },
    ...
    /**
     * テキストエリアのクリックハンドラーを一定時間無効にする
     * @param {Integer} delay 無効にする時間(ミリ秒)
     * @method startIgnoreClickTimer
     */
    startIgnoreClickTimer: function(delay){
      var self = this;
    ...
    /**
     * テキストエリアのクリックハンドラーを無効にすべきか否かを返す
     * @method isIgnoreClick
     */
    isIgnoreClick: function(){
      return this.ignore_click;
    },

    /**
     * 遷移処理を実行
     * @method execTransition
     */
    execTransition: function() {
      if (!!this.stat.auto_transition_func) {
        this.stat.auto_transition_func();
      }
    },
