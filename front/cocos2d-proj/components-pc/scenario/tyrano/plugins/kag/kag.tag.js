...
tyrano.plugin.kag.ftag = {
    ...
    isReadyToNextOrder : function() {

        return this.kag.stat.is_adding_text === false;
    ...
    nextOrder : function() {
        ...
        if (!this.isReadyToNextOrder()) {
            //TODO:
            // nextOrderが文字送り中に呼ばれて文字送りのバッファを壊す不具合への対応
            // 潜在的に、文字送りバッファを知らずにnextOrderを発行する処理が多数存在するので、
            // 本対応でそれらのケース全てに対し回避を行う
            // 根本対策としては、nextOrderを様々な箇所から直接呼び出してる構造を変えるべき
            setTimeout(function () { this.nextOrder(); }.bind(this), 1000);
            return false;
        }
        ...
        try {
            ...
            if (kh._needCopy) {
                ...
                if (tag.name == "endscript")
                    this.iscript = false;
                if (!this.iscript) {
                    if (tag.pm.val != undefined && tag.name == "text" && !kh.copyLock) {
                        var name = $(".chara_name_area").html();
                        var findex = this.current_order_index;
                        var ftext = "";
                        var temptag = $.cloneObject(this.array_tag[findex]);
                        while (temptag.name != "l") {
                            if (temptag.name == "text") {
                                ftext += '<br>' + temptag.pm.val;
                            }
                            findex++;
                            temptag = $.cloneObject(this.array_tag[findex]);
                        }
                        kh.copyLock = true;
                        kh.copyTextToClipboard('【' + name + '】' + ftext);
                    }
                    if (tag.name == "l" && kh.copyLock) {
                        kh.copyLock = false;
                    }
                }
            }
    ...
    setAutoTransitionTimer: function(delay) {
      // クリックで次のテキストに遷移した時にタイマーが実行されている事があるのでリセットする
      this.kag.stopTimer();
      this.kag.startTimer(delay, function() { this.kag.ftag.nextOrder(); }.bind(this));
    },

    /**
     * テキストの文字数からオート再生のタイマーを設定
     * @method setAutoTransitionTimerFromTextLength
     */
    setAutoTransitionTimerFromTextLength: function() {
      this.kag.ftag.setAutoTransitionTimer(this.kag.stat.current_message_str.length * 250); // テキストの文字数×0.25秒
    }
...
tyrano.plugin.kag.tag.text = {
    ...
    showMessage : function(message_str) {
        ...
        (function(jtext) {
            ...
            //メッセージ領域を取得
            var current_span = that.kag.getMessageCurrentSpan();
...
tyrano.plugin.kag.tag.l = {
    ...
    start : function() {
        ...
        if (this.kag.stat.is_skip == true) {
          //スキップ中の場合は、nextorder
          this.kag.ftag.nextOrder();
        } else if (this.kag.stat.is_auto == true && this.kag.stat.play_se == false) {
...
tyrano.plugin.kag.tag.p = {
    ...
    start : function() {
        ...
        if (this.kag.stat.is_skip == true) {
            ...
            // オート再生かつボイスがなしの場合は、テキスト1文字につき0.25秒待機し、次のテキストに遷移する
            this.kag.ftag.setAutoTransitionTimerFromTextLength();
        }
    }
};
    ...
    start : function(pm) {
        ...
        if (pm.layer != "base") {
            ...
            if ($.isHTTP(pm.storage)) {
                ...
                strage_url = window.resourceDir + "/" + folder + "/" + pm.storage;
            }
...
tyrano.plugin.kag.tag.quake = {
    ...
    start : function(pm) {
        // TODO+ IOSでは画面が大きくなってしまうのでquakeを無視するように修正
...
tyrano.plugin.kag.tag.bg = {
    ...
    start : function(pm) {
        ...
        var storage_url = window.resourceDir + "/bgimage/"+pm.storage;
        if($.isHTTP(pm.storage)){
            storage_url = pm.storage;  
        }
