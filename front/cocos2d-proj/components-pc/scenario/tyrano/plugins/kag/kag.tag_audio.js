...
tyrano.plugin.kag.tag.playbgm = {
    ...
    /** ボイスの再生時間から自動遷移タイマーの時間を設定
     * @method setAutoTransitionTimerFromVoiceDuration
     * @param {Object} audio
     */
    setAutoTransitionTimerFromVoiceDuration: function(audio) {
      ...
      var voiceDuration = $.getBrowser() != "msie" ? audio.buffer.duration : audio.duration;

      this.kag.ftag.setAutoTransitionTimer(voiceDuration * 1000);
    },
    ...
    play:function(pm){
       ...
       if($.isHTTP(pm.storage)){
         ...
         if (pm.target == "se") {
            storage_url = window.seDir + "/"+target+"/" + pm.storage;
         } else {
            storage_url = window.resourceDir + "/"+target+"/" + pm.storage;
         }
    	 }

       //音楽再生
       boombox.load(storage_url, {src: [{media: "audio/mp3", path: storage_url}]}, function(err, audio_obj) {
         if (err) {
          //alert(err);
          console.warn(err);
          return;
         ...
         // boombox.jsにonEndedイベントがあるが、ボイスの再生が完了しないとイベントが発行されないため、
         // ボイスの再生途中でサウンドをOFFにすると再生完了を検知出来ない
         // そのため、ボイスの再生時間からタイマーを設定して自動遷移するようにする
         if (pm.target === "se") {
           that.setAutoTransitionTimerFromVoiceDuration(audio_obj);
         }

         var volume = target === "bgm" ? window.bgmVolume : window.voiceVolume;
         audio_obj.volume(volume || 0);

         if (pm.loop == "true") {
          audio_obj.setLoop(boombox.LOOP_ORIGINAL);
         }

        if(target ==="bgm"){
            that.kag.tmp.map_bgm[pm.storage] = audio_obj;
            that.kag.stat.current_bgm = pm.storage;
            ...
            that.kag.stat.current_se = pm.storage;
        ...
        if(pm.target=="bgm" && that.kag.stat.play_bgm == false){
          audio_obj.pause();
        } else if(pm.target=="se" && that.kag.stat.play_se == false){
          audio_obj.pause();
        } else {
          audio_obj.play();
        }
...
tyrano.plugin.kag.tag.playse = {
    ...
    start:function(pm){
        // SEが再生可能な状態で無ければ、SEは再生しない
        this.kag.stat.play_se = true;
        if (this.kag.stat.is_playing_se == false) {
          this.kag.stat.play_se = false;
        }
        if(pm.clear == "true"){
