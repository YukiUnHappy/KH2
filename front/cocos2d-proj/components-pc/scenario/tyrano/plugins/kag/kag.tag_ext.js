...
tyrano.plugin.kag.tag.chara_ptext ={
    ...
    start:function(pm){
        ...
        window.TYRANO_LOG.next();
        if(pm.name==""){
            $("."+this.kag.stat.chara_ptext).html("");
            window.TYRANO_LOG.setChara("");
        }else{
            ...
            //キャラクター名出力
                ...
                window.TYRANO_LOG.setChara(cpm.jname);
            
            }else{
                //存在しない場合はそのまま表示できる
                 $("."+this.kag.stat.chara_ptext).html(pm.name);
                window.TYRANO_LOG.setChara(pm.name);
            }
        }
        ...
        if(pm.face !=""){
       		...
       		$("."+pm.name).attr("src", window.resourceDir + "/fgimage/"+storage_url);
...
tyrano.plugin.kag.tag.chara_new ={
    ...
    start:function(pm){
        ...
        var storage_url = window.resourceDir + "/fgimage/"+pm.storage;
...
tyrano.plugin.kag.tag.chara_show ={
    ...
    start:function(pm){
        ...
        var storage_url = window.resourceDir + "/fgimage/"+cpm.storage;
...
tyrano.plugin.kag.tag.chara_mod ={
    ...
    start:function(pm){
       ...
       //変更する際の画像が同じ場合は、即時表示
       if($(".layer_fore").find("."+pm.name).attr("src") == window.resourceDir + "/fgimage/"+storage_url){
           chara_time = "0";
       }
       ...
       if(chara_time !="0"){
           var j_new_img = $(".layer_fore").find("."+pm.name).clone();
            j_new_img.attr("src", window.resourceDir + "/fgimage/"+storage_url);
            ...
            $(".layer_fore").find("."+pm.name).attr("src", window.resourceDir + "/fgimage/"+storage_url);
            this.kag.ftag.nextOrder();
       }
