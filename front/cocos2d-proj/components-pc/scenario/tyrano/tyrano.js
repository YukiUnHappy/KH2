...
// - 話者の名前を記録
// - ストーリーを記録
//   - 1セリフ複数にわかれることがある => 話者の名前が変わったら新しいセリフとする(next() と setChara()を叩く )
window.TYRANO_LOG = {
  currentCharaName: null,
  list: [],
  pos: -1,
  next: function() {
    this.pos++;
  ...
  setChara: function(name) {
    this.list[this.pos].name = name;
  },
  append: function(text) {
    this.list[this.pos].text = [this.list[this.pos].text, text, "\n"].join("");
  },
  get: function() {
    return this.list;
  }
